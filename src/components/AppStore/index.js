...
class AppStore extends Component {
  ...
  getTabAppItemsList = searchedApps => {
    const {activeTabId} = this.state
    const activeTabAppsList = searchedApps.filter(
      eachItem => eachItem.category === activeTabId,
    )
    return activeTabAppsList
  }
  ...
  render() {
    ...
    const activeTabAppsList = this.getTabAppItemsList(searchResults)
          ...
          <ul className="tabs-container ">
            {tabsList.map(eachItem => (
              <TabItem
                ...
                key={eachItem.tabId}
                isActiveTabId={eachItem.tabId === activeTabId}
              />
            ))}
          </ul>
          <ul className="apps-container">
            {activeTabAppsList.map(eachApp => (
